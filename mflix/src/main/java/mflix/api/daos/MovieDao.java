...
//        // match stage to find movie
		...
		// start of comments query
		List let = Arrays.asList(new Variable<>("movieid", "$_id"));
		List<Bson> lookupPipeline = new ArrayList<>();

//        List<Document> asList = Arrays.asList(new Document("$match", 
//                new Document("$expr", 
//                new Document("$eq", Arrays.asList("$movie_id", "$$id")))));
	...
	public List<Document> getMoviesByCountry(String... country) {
		...
		System.out.println("--------------");
		Bson queryFilter = new Document("countries", new Document("$all", countryList));
